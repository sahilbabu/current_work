    ...
    <article id="content" class="clearfix"> 
        ...
        <div class="postsContainer classic">
           ...
           print  render($page['content']);
                $blog_data = $page['content']['system_main']['nodes']; 
            /*
                foreach ($blog_data as $blog) {
                    $entity  =  $blog['body']['#object'];
                    $title  =  $blog['body']['#object']->title;
                    $nid  =  $blog['body']['#object']->nid;
                    $comment_count  =  $blog['#node']->comment_count;
                    $name  =  $blog['#node']->name;
                    $field_category  =  $blog['body']['#object']->field_category['und'];
                 //   $image  =  $blog['body']['#node']['field_image']['und'][0]['filename'];
                 //   $blog_image =   image_style_url($style, $node->field_image['und'][0]['uri']);
                    $image  =  $blog['body']['#object']->field_image['und'][0]['uri'];
                    $date = $blog['body']['#object']->created;
                    $date = date("jS F Y",$date); 
          
                    $blog_image =   file_create_url($image);
                    $short_content = $blog['body'][0]['#markup'];
                 //   $short_content = $blog['body']['#items'];
                 //   print $short_content;
                    $options = array('absolute' => TRUE);
                    $url = url('node/' . $nid, $options);
                    
                    $links = $blog['links']['node']['#links']['node-readmore'];
                    $read_more = '<a href="'.url($links['href'], $options).'" class="more nav-next" rel="'.$links['attributes']['rel'].'"  title="'.$links['attributes']['title'].'">'.$links['title'].'</a>';
                   // print $read_more;
                    
            ?>
            
            <pre><?php //  print_r($links); exit(); ?></pre>
         <article class="post-167 post type-post status-publish format-gallery hentry category-media tag-gallery post clearfix">

             <a class="pTitle clearfix" href="<?php print $url; ?>">
                    <h2 class="icon-none"><?php print $title; ?></h2>
                </a>

                <div class="flexslider pMedia folio">
                    <ul class="slides"><li data-caption="">
                            <img src="<?php //  print $blog_image; ?>" width="700" height="350" alt="" />
                        </li></ul>
                </div>
                <div class="content">
...
<!--   
            ...
            <nav class='pagination'>
                <a class='more nav-prev' href='http://demo.krownthemes.com/goodwork/blog-classic/page/2/'>Older posts</a>
                <a class='hidden' href='http://demo.krownthemes.com/goodwork/blog-classic/'>Newer posts</a>
            </nav>
